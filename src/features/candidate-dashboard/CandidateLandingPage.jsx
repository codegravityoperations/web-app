import { useState } from "react";
import { API, apiFetch, clearTokens } from "../../apiClient";
import "./CandidateLandingPage.css";

export default function CandidateLandingPage({
  auth,
  onEditProfile,
  onLogout,
}) {
  const [loggingOut, setLoggingOut] = useState(false);

  const candidateName =
    auth?.firstName ||
    auth?.fullName ||
    auth?.email?.split("@")[0] ||
    "Candidate";

  const handleLogout = async () => {
    setLoggingOut(true);

    try {
      await apiFetch(API.logout, {
        auth: true,
      });
    } catch (error) {
      console.error("Logout request failed:", error);
    } finally {
      clearTokens();
      onLogout();      
    }
  };

  return (
    <div className="candidate-dashboard-page">
      <header className="candidate-dashboard-header">
        <div className="candidate-dashboard-brand">
          <div className="candidate-dashboard-logo">
            <svg
              width="18"
              height="18"
              viewBox="0 0 24 24"
              fill="none"
              aria-hidden="true"
            >
              <path
                d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
          </div>

          <div>
            <h2>Code Gravity</h2>
            <p>Candidate Portal</p>
          </div>
        </div>

        <div className="candidate-dashboard-user">
          <div className="candidate-dashboard-avatar">
            {candidateName.charAt(0).toUpperCase()}
          </div>

          <div className="candidate-dashboard-user-details">
            <strong>{candidateName}</strong>
            <span>{auth?.email || "No email available"}</span>
          </div>

          <button
            type="button"
            className="candidate-dashboard-logout"
            onClick={handleLogout}
            disabled={loggingOut}
          >
            {loggingOut ? "Signing out..." : "Sign out"}
          </button>
        </div>
      </header>

      <main className="candidate-dashboard-content">
        <section className="candidate-dashboard-welcome">
          <div>
            <span className="candidate-dashboard-label">
              Candidate Dashboard
            </span>

            <h1>Welcome back, {candidateName}!</h1>

            <p>
              Review your account information and keep your candidate profile
              up to date.
            </p>
          </div>

          <button
            type="button"
            className="candidate-dashboard-edit-button"
            onClick={onEditProfile}
          >
            Edit Profile
          </button>
        </section>

        <section className="candidate-dashboard-info-grid">
          <article className="candidate-dashboard-info-card">
            <div className="candidate-dashboard-card-icon purple">
              <svg
                width="23"
                height="23"
                viewBox="0 0 24 24"
                fill="none"
                aria-hidden="true"
              >
                <path
                  d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8z"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </div>

            <div>
              <span className="candidate-dashboard-card-label">
                Account Type
              </span>

              <strong>{auth?.userType || "CANDIDATE"}</strong>

              <p>Your registered portal account type.</p>
            </div>
          </article>

          <article className="candidate-dashboard-info-card">
            <div className="candidate-dashboard-card-icon blue">
              <svg
                width="23"
                height="23"
                viewBox="0 0 24 24"
                fill="none"
                aria-hidden="true"
              >
                <path
                  d="M4 4h16v16H4V4zM4 8l8 5 8-5"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </div>

            <div>
              <span className="candidate-dashboard-card-label">
                Email Address
              </span>

              <strong className="candidate-dashboard-card-value">
                {auth?.email || "Not available"}
              </strong>

              <p>The email associated with your account.</p>
            </div>
          </article>

          <article className="candidate-dashboard-info-card">
            <div className="candidate-dashboard-card-icon green">
              <svg
                width="23"
                height="23"
                viewBox="0 0 24 24"
                fill="none"
                aria-hidden="true"
              >
                <path
                  d="M9 12l2 2 4-4M12 22a10 10 0 100-20 10 10 0 000 20z"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </div>

            <div>
              <span className="candidate-dashboard-card-label">
                Account Status
              </span>

              <strong className="candidate-dashboard-active">
                Active
              </strong>

              <p>Your candidate account is active.</p>
            </div>
          </article>
        </section>

        <section className="candidate-dashboard-profile-section">
          <div className="candidate-dashboard-section-heading">
            <div>
              <h2>Your account information</h2>
              <p>
                This information comes from your authenticated login session.
              </p>
            </div>

            <button
              type="button"
              className="candidate-dashboard-secondary-button"
              onClick={onEditProfile}
            >
              Update Profile
            </button>
          </div>

          <div className="candidate-dashboard-details">
            <div className="candidate-dashboard-detail">
              <span>Email</span>
              <strong>{auth?.email || "Not available"}</strong>
            </div>

            <div className="candidate-dashboard-detail">
              <span>Candidate ID</span>
              <strong>{auth?.businessId || "Not available"}</strong>
            </div>

            <div className="candidate-dashboard-detail">
              <span>Role</span>
              <strong>{auth?.role || "ROLE_CANDIDATE"}</strong>
            </div>

            <div className="candidate-dashboard-detail">
              <span>Token Type</span>
              <strong>{auth?.tokenType || "Bearer"}</strong>
            </div>
          </div>
        </section>

        <section className="candidate-dashboard-actions-section">
          <div className="candidate-dashboard-section-title">
            <h2>Quick actions</h2>
            <p>Manage your candidate portal account.</p>
          </div>

          <div className="candidate-dashboard-actions-grid">
            <button
              type="button"
              className="candidate-dashboard-action-card"
              onClick={onEditProfile}
            >
              <div className="candidate-dashboard-action-icon">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  aria-hidden="true"
                >
                  <path
                    d="M12 20h9M16.5 3.5a2.12 2.12 0 013 3L7 19l-4 1 1-4L16.5 3.5z"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </div>

              <div>
                <h3>Edit Profile</h3>
                <p>Update your personal and contact information.</p>
              </div>

              <span className="candidate-dashboard-arrow">→</span>
            </button>

            <button
              type="button"
              className="candidate-dashboard-action-card disabled"
              disabled
            >
              <div className="candidate-dashboard-action-icon">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  aria-hidden="true"
                >
                  <path
                    d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8zM14 2v6h6M8 13h8M8 17h8"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </div>

              <div>
                <h3>My Applications</h3>
                <p>This feature will be available in a future update.</p>
              </div>
            </button>

            <button
              type="button"
              className="candidate-dashboard-action-card disabled"
              disabled
            >
              <div className="candidate-dashboard-action-icon">
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  aria-hidden="true"
                >
                  <path
                    d="M9 6V4a2 2 0 012-2h2a2 2 0 012 2v2M3 9h18M5 6h14a2 2 0 012 2v11a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2z"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </div>

              <div>
                <h3>Browse Jobs</h3>
                <p>This feature will be available in a future update.</p>
              </div>
            </button>
          </div>
        </section>
      </main>
    </div>
  );
}